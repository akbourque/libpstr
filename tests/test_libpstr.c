...
static void test_substring(void) {
    ...
    libpstr_slice_t slb = libpstr.pstr.substring(s, 1, 4);
    assert(slb.ptr && slb.len == 4);
    libpstr_slice_t slc = libpstr.slice.from_pstr(s);
    libpstr_slice_t sub1 = libpstr.slice.substring(slc, 1, 4);
    assert(sub1.ptr);
    assert(sub1.len == 4);
    assert(memcmp(sub1.ptr, "\xF0\x9F\x9A\x80", 4) == 0);

    // Assert protective boundary panics
    ASSERT_PANIC(libpstr.slice.substring(slc, 10, 1));
    ASSERT_PANIC(libpstr.slice.substring(slc, 1, 10)); // Strict 1.0.0 boundary enforcement
...
static void test_defensive_panic_boundaries(void) {
    ...
    libpstr_pstr_t *s = libpstr.pstr.from_cstr("A🚀B");
    libpstr_slice_t slc = libpstr.slice.from_pstr(s);
    ASSERT_PANIC(libpstr.slice.substring(slc, 2, 2)); 
    ASSERT_PANIC(libpstr.slice.substring(slc, 10, 1));
